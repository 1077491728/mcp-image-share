...
# ---------- 配置（env 可覆盖）----------
DEFAULT_BASEURL = os.environ.get("MICU_BASEURL", "https://new.misscuai.help")
...
PRO_MODEL = "gpt-image-2"
NONPRO_MODEL = "gpt-image-2"

# 网页里实测出的阈值：此端点所有请求统一输出 ~1.57MP，不支持真 2K/4K
HIGH_RES_EDGE = 1600
# 图生图代理后端实测：所有尺寸统一 ~1.57MP
EDITS_MAX_EDGE = 1536

# 实测像素映射（请求 size -> 实际输出 size），所有输出固定 ~1.57MP
...
# 宽高比保持正确，总像素锁定 ~1.57MP

VALID_SIZES_1K = {"1024x1024", "1280x720", "720x1280", "1024x1536", "1536x1024"}
# 此端点不支持 pro，所有请求统一输出 ~1.57MP；2K/4K 请求会被压缩到 1.57MP
VALID_SIZES_2K = {"2048x2048", "2048x1152", "1152x2048", "1920x1080", "1080x1920"}
...
def server_info() -> dict[str, Any]:
    ...
    return {
        ...
        "available_models": ["gpt-image-1", "gpt-image-2", "grok-imagine-image-lite"],
        ...
        "size_rules": {
            ...
            "output_behavior": (
                ...
                "不支持 pro 模型，不支持真 2K/4K 输出。"
            ),
            "actual_pixel_mapping": {
                "1024x1024": "1254x1254 (1.57MP)",
                "1280x720": "1672x941 (1.57MP)",
                "720x1280": "941x1672 (1.57MP)",
                "1024x1536": "1024x1536 (1.57MP)",
        ...
        "safety_constraints": {
            ...
            "input_image_validation": "所有输入图按 magic bytes 校验为 PNG/JPEG/WebP/GIF；非图片立即拒",
            "response_size_limit": f"远端响应 ≤{MAX_RESPONSE_BYTES//1024//1024}MB；超过中断不落盘",
            "base_url_locked": "base_url 锁在启动期 MICU_BASEURL env，运行期 tool 不接受参数",
        },
        "recommended_sizes": {
            "推荐_宽高比保持": sorted(VALID_SIZES_1K),
            "可用但输出仍1.57MP": sorted(VALID_SIZES_2K | VALID_SIZES_4K),
            "tip": "所有尺寸输出固定 ~1.57MP，宽高比正确。推荐直接用 1K 档。",
        },
        "capability_matrix": {
            "image_generate": "可用，~30-45s，N>1 自动 5 并发",
            "image_edit": "可用，edits multipart + 可选 alpha mask",
            "image_batch_edit": "可用，5 并发",
            "image_multi_reference": "可用，2-10 张参考图融合",
        },
        "retry_policy": {
            "retryable_status": list(RETRYABLE_STATUS),
            "schedule": (
                "网络层异常先免费等待 2s 重试 1 次；之后可恢复错误按 "
                "4s + jitter、8s + jitter 最多再重试 2 次"
            ),
        },
        "known_issues": {
            "720x1280": "偶发 HTTP 500（上游不稳定），重试通常成功",
            "pro_model": "此端点不支持 gpt-image-2-pro，所有请求走 gpt-image-2",
        },
    }
